...
<%
'//===================================================
' 动网论坛验证码 8.2提取版，可独立用于各个ASP系统
' 作者	Dv.HxyMan,www.ssoso.com debiangrub提取
' 更新	2008.4.29
...
Const DvCodeFile = "DV_getcode.asp"'这里改成你要的文件名，如果你把dv_getcode.asp文件名变了，这里跟着变一下就好了。
...
sub head()
response.write "<html><head><TITLE>增强验证码管理页面 由动网中提取UTF-8版 by debiangrub www.ssoso.com</TITLE></head><body>"
end sub

Function ReadTextFile(fileName)
    On Error Resume Next
        'Response.Write Server.MapPath(ScriptPath&fileName)
        DvStream.charset="utf-8"
        DvStream.Mode = 3
        DvStream.open()
        DvStream.LoadFromFile(Server.MapPath(fileName))
    ...
    If Err Then
        ...
        PageEnd()
        Response.Clear
        Response.Write fileName & "文件不存在！请检查,或者恢复官方模板数据！"
        Response.End
    End If
End Function
...
Function GetScriptContent()
    ...
    str = ReadTextFile("CaptchaTemplate.txt")
...
If "preview"=request("action") Then
	str = ReadTextFile("CaptchaPreview.asp")
Else
	str = ReadTextFile(DvCodeFile)
End If 
Set re=new RegExp
re.IgnoreCase =true
re.Global=True
...
End If 
Set re=Nothing
Set xmlObj=Server.CreateObject("msxml2.FreeThreadedDOMDocument"& MsxmlVersion)
