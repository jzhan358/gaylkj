                            ...
                            <TD width="100%">
                              ...
                              border=0>
                                ...
                                <TD bgColor=#eaeaea height=1></TD>
                                                        ...
                                                        <%
if request.QueryString("classtype") <> "" then
pro_type=replace(request.QueryString("classtype"),"'","")
else
pro_type=""
end if

if isnull(pro_type) then
sql="select * from P_info order by p_date desc"
else
set typesql=conn.execute("select p_type from p_class where p_type_e='"&pro_type&"'")
if not typesql.eof then
sql="select * from P_info where p_type='"&typesql("p_type")&"' order by p_date desc"
else
sql="select * from P_info order by p_date desc"
end if
typesql.close
...
%>
                                  ...
                                  <TD height=5></TD>
                                                    ...
                                                    <tr> 
                                                      <td valign="top">
														The display£ºProducts show - ->Total <%=rs.recordcount%> pcs, Total <%=rs.pagecount%>  page,This is the <%=pagecount%> page, 12 pcs of products per page      
                                                      </td>
                                                    </tr>
                                                    <tr> 
					...
					%>
                                                            <td align="center" class="line" width="539"> 
                                                              <table width="131" border="0" cellspacing="0" cellpadding="0">
                                                                <tr> 
                                                                  <td height="88" align="center" valign="bottom" style="border:1px #00CC99 solid"><a href="eshowProDetail.asp?ProID=<%=rs("p_id")%>"><img src="<%=rs("p_pic")%>" width="110" height="150" border="0"></a></td>
                                                                </tr>
                                                                <tr> 
                                                                  <td height="50" align="center" class="font9"> 
                                                                    Name£º<A href="eshowProDetail.asp?ProID=<%=rs("p_id")%>" class="money"><%=left(rs("p_name_e"),6)+".."%></A><br>
                                                                    Item£º<%=rs("p_spec_e")%> 
                                                                  </td>
                                                                </tr>
                                                              </table></td>
					 ...
					 %>
						...
						%>
                                                                  <option value="?page=<%=i%>&classType=<%=classType%>&smallclass=<%=smallclass%>&key=<%=key%>&type=<%=request("type")%>" <% if pagecount = i then response.Write("selected")%>><%=i%> Page</option>
                                                                  <%
						next
						%>
                                                                </select>  
