...
if m_title="" then
  founderr=true
  errmsg=errmsg+"<br>"+"<li>title not null!"
else
  rs("m_title")=m_title
End if
if m_name="" then
...
if m_text="" then
  founderr=true
  errmsg=errmsg+"<br>"+"<li>Content not null"
else
   rs("m_text")=m_text
End if
...
set rs=Nothing
  closedatabase
 Response.write "<script language = 'javascript'>alert('succeed!');window.document.location.href='eGuest.asp';</script>"
else
Call diserror()
Response.write "<script language = 'javascript'>alert('message not completed!');window.document.location.href='javascript:history.back()';</script>"

response.End
End if
else
response.Redirect("eGuest.asp")
End if
%>
