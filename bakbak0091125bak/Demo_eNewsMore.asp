              ...
              <TR>
                <TD vAlign=top width=61 height="318">
                  <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
                    <TBODY>
                    <TR>
                      <TD><IMG src="index/Top_Bn_Left.gif" border=0></TD></TR>
                    <TR>
                      <TD height=20><!-- //左栏菜单 -->
...
if newstype="" then
 sql="select news.news_id,news_title,news_date,news_type,news_content from news,news_class where news.news_class_id=news_class.news_id and news_language=1 order by news_date DESC"  
else
 sql="select news.news_id,news_title,news_date,news_type,news_content from news,news_class where news.news_class_id=news_class.news_id and news_language=1 "
...
%>
          <tr> 
            <td width="520" height="22"><img src=images/none.gif width="1" height="17" border=0><img src=images/i4.gif width="10" height="12" border='0'>&nbsp;[<a href="?newstype=<%=rs("news_type")%>"> 
              <%=rs("news_type")%></a> ]  
			  <a href="eShowNews.asp?news_id=<%=rs("news_id")%>"> 
              <font color="#CC6600"><%=left(rs("news_title"),30)%></font> </a></td>
            <td width="187" align="center"><%=rs("news_date")%>　</td>
          </tr>
            <%
i=i+1
rs.movenext
...
%>
          <tr> 
            <td colspan="2" height="22" align="center">There is no news at present !</td>  
          </tr>
          <%End if
End if
'开始分页
if Not request("type")=1 then%>
          <form name="form1" method="post" action="Demo_enewsmore.asp">
            <tr align="center"> 
              <td height="30" colspan="2">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;NEWS[<font color="#FF6666"><%=totalnews%></font>]　PAGE[<font color="#FF6666"><%=totalpages%></font>]   
                <%
if CurrentPage<2 then
response.write "<font color='000000'>FIRST LAST</font> "
else
response.write "<a href=?page=1&newstype="&request("newstype")&">FIRST</a> "
response.write "<a href=?page="&CurrentPage-1&"&newstype="&request("newstype")&">LAST</a> "
End if
if totalpages-currentpage<1 then
response.write "<font color='000000'>NEXT END</font>"
else
response.write "<a href=?page="&CurrentPage+1&"&newstype="&request("newstype")
response.write ">NEXT</a> <a href=?page="&totalpages&"&newstype="&request("newstype")&">END</a>"
End if
end if
%>
...
%>
                  <option value=<%=i%> selected><%=i%>PAGE</option>
                  <%else%>
...
next  
 %>
                </select> <input type="submit" name="Submit2" value="GO" class="button">   
