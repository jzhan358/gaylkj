...
if company="" then
  founderr=true
  errmsg=errmsg+"<br>"+"<li>客户名称不能为空！"
else
  rs("company")=company
End if
if addr="" then
  founderr=true
  errmsg=errmsg+"<br>"+"<li>联系地址不能为空！"
else
  rs("addr")=addr
End if
if tel="" then
...
if num="" then
  founderr=true
  errmsg=errmsg+"<br>"+"<li>订购数量不能为空！"
elseif Not isInteger(num) then
  founderr=true
  errmsg=errmsg+"<br>"+"<li>正确填写订购数量！"
else
   rs("num")=num
End if
...
set rs=Nothing
closedatabase
Response.write "<script language = 'javascript'>alert('你已经成功订购此产品,我们会尽快处理!');window.document.location.href='Order.asp';</script>"
else
Call diserror()
Response.write "<script language = 'javascript'>alert('请正确填写订购信息!');window.document.location.href='javascript:history.back()';</script>"
